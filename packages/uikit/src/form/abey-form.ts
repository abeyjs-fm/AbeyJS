...
import type { FormObjectTab, FormSlice, FormViewDef, ViewField, ViewTheme } from "./form-types.js";
...
/** Valores “vacíos” por lista de campos (raíz o un objeto anidado). */
function clearedFieldsRecord(fields: ViewField[], iv: Record<string, unknown>): Record<string, unknown> {
...
export class AbeyFormElement extends HTMLElement {
  static get observedAttributes(): string[] {
    return ["intentsubmit", "runtimepath", "flow", "configpath", "extraspath", "actions"];
  }

  #config: AbeyFormConfig | null = null;
  /** Schema efectivo al enviar: el explícito o el inferido desde `ViewField` cuando aplica. */
  #effectiveSchema: ZodType<Record<string, unknown>> | undefined;
  /** Clases de `config.theme.className` replicadas en el host para que apliquen reglas como `.abey-form.abey-form--full`. */
  ...
  /**
   * Lee un valor del store del form. Por defecto lee del documento completo (`values` + extras).
   * `path` usa notación con puntos (ej: `"items"` o `"totales.subtotal"`).
   */
  getStoreValue(path?: string): unknown {
    const base = { ...(this.#slice.value ?? {}), ...(this.#storeExtra ?? {}) } as Record<string, unknown>;
    const p = (path ?? "").trim();
    if (!p) return base;
  ...
  /**
   * Setea un valor en el store extra del form (no depende de inputs en el DOM).
   * Útil para rejillas/tablas (`items[]`) u otras estructuras que no son `ViewField[]`.
   */
  ...
  /** Sincroniza estado remoto (errores, valores, toggle opcionales). */
  ...
  #getRuntime(): OmegaRuntime | null {
    const path = (this.getAttribute("runtimepath") ?? this.#runtimePath).trim() || "__abeyRuntime";
  ...
  #rebuild(): void {
    ...
    this.#effectiveSchema =
      cfg.schema ??
      (cfg.inferBasicSchema === false ? undefined : inferBasicFormSchema({ fields: cfg.fields, tabs: cfg.tabs }));
      ...
      onResetSlice: () => {
        const next: FormSlice = {
          ...
          includeOptionalFields:
            cfg.showOptionalFieldsToggle === true ? (cfg.includeOptionalFields ?? true) : undefined,
        };
        this.#slice = next;

        // Reset store extras (e.g. `items`) back to initial `extraspath`.
        this.#storeExtra = {};
        const extrasPath = (this.getAttribute("extraspath") ?? "").trim();
        if (extrasPath) {
          const extras = resolveGlobalPath(extrasPath) as Record<string, unknown> | undefined;
        ...
        this.#dispatchFormIntent(this.#config?.resetIntent, { values: next.value });
        return next;
      },
    });
    ...
    // Slot automático de hijos declarativos en paneles de tabs (sin que el usuario manipule DOM).
    // Ej: <abey-line-items tab="items" name="items"></abey-line-items>
    this.#autoSlotDeclarativeChildren(cfg);
    // Al igual que con otros custom elements, `connectedCallback` puede disparar durante el parsing
    // antes de que se inserten todos los hijos declarativos. Un segundo pase en microtask evita tabs vacíos.
    queueMicrotask(() => {
      if (!this.isConnected) return;
      this.#autoSlotDeclarativeChildren(cfg);
    });
    // Si aún aparecen hijos declarativos después (HMR/parsing), observar y reintentar hasta estabilizar.
    this.#autoslotObserver?.disconnect();
    ...
    surface.onSubmit(() => {
      ...
      this.#slice = { ...this.#slice, status: "idle", errorMessage: undefined };
      this.#surface.update(this.#slice);
      const rec = { ...this.#surface.readFormData(this.#slice), ...(this.#storeExtra ?? {}) };
      const schema = this.#effectiveSchema;
      if (schema) {
        const p = safeParseWithErrors(schema, rec);
        if (!p.success) {
          const fe = (cfg.tabs?.length ?? 0) > 0 ? fieldErrorsToDottedMap(p.fields) : fieldErrorsToMap(p.fields);
          this.#slice = { ...this.#slice, fieldErrors: fe, status: "error" };
          this.#surface.update(this.#slice);
          this.dispatchEvent(
          ...
          this.#dispatchFormIntent(this.#config?.invalidIntent, { fieldErrors: fe, record: rec });
          return;
        }
        // Zod por defecto "strippea" keys desconocidas; re-agregar extras para que el UI reciba `items`, etc.
        const merged = { ...(p.data as Record<string, unknown>), ...(this.#storeExtra ?? {}) };
  ...
  #autoSlotDeclarativeChildren(cfg: AbeyFormConfig): void {
    const kids = Array.from(this.children).filter((el) => el !== this.#surface?.section);
    for (const el of kids) {
      ...
      // Si el componente ya está en el panel, no hacer nada.
      ...
      slotHostIntoAbeyFormTabPanel(this, el, panelIndex, {
        ...
        titleClassName: (el.getAttribute("tab-title-class") ?? "").trim() || undefined,
      });
    }
  }
  ...
  #emitSuccess(values: Record<string, unknown>): void {
    if (!this.#surface || !this.#config) return;
    this.#slice = { ...this.#slice, fieldErrors: undefined, errorMessage: undefined, status: "success" };
    this.#surface.update(this.#slice);

    const intentName = (this.getAttribute("intentsubmit") ?? this.#config.submitIntent ?? "").trim();
    const rt = this.#getRuntime();
    ...
    if (intentName && rt && typeof rt.dispatch === "function" && !flowOff) {
      void rt.dispatch(intentOf(intentName, { [key]: values }), { source: "abey-form" });
    }
  ...
  static define(tagName = "abey-form"): void {
    if (!customElements.get(tagName)) {
      customElements.define(tagName, AbeyFormElement as CustomElementConstructor);
    }
  }
}
