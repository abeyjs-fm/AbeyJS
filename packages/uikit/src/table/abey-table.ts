...
import type {
  ...
  AbeyTableStatusTone,
} from "./abey-table.types.js";

type AbeyTableColumnJson = {
  key: string;
  header?: string;
  width?: string;
...
type AbeyTableActionJson = {
  id: string;
  label: string;
  eventName?: string;
};

type AbeyTableActionsPayload = {
...
function clear(node: ParentNode) {
  while (node.firstChild) node.firstChild.remove();
}

function isTemplatePlaceholder(v: string | null): boolean {
  const s = (v ?? "").trim();
...
function interpolateString(tpl: string, row: any): string {
 ...
 */
function enhanceCellTemplateImages(root: ParentNode): void {
...
function renderTemplateFragment(tpl: HTMLTemplateElement, row: any, cellImageDefaults: boolean): DocumentFragment {
...
function appendTrustedCellContent(host: HTMLElement, value: AbeyTableCell) {
...
function appendUntrustedCellContent(host: HTMLElement, value: unknown) {
  if (Array.isArray(value)) {
