...
AbeyJs is the stack we built for SPAs with a **real DOM**, **OmegaRuntime**, and **intents** as the backbone. There is no central VDOM: the compiler turns your **`.view.html`** into elements and the binder updates only what changes. Coming from React or Vue, think *web components + message contract + product shell*, not another diffing runtime.
...
- **OM** (Omega markup): HTML with `{{ }}`, `[prop]`, `(click)` and `@if` / `@for` blocks, compiled by Vite (`@abeyjs/compiler`).
...
| `@abeyjs/view` | Shell, routes, `@AbeyComponent`, lists/forms, default HTML safety, Omega theme. |
...
If you are opening the docs site cold, follow **`/guides/quick-start`** → **`/guides/bootstrap-shell`** → **`/guides/routing`** → **`/guides/abey-component`**. Omega and CLI can slot in after you have a compiling screen; **`/guides/vision`**, OpenAPI, and tables assume that map is clear.
