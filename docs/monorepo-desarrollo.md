...
To fix that for **known** routes, **`docs/web/vite.config.ts`** registers a small post-build plugin that, after **`vite build`**, duplicates the generated root **`dist/index.html`** into **`dist/<app-path>/index.html`** for every entry in **`docs/web/vite-doc-spa-paths.ts`**. Paths are **application paths** (leading **`/`**, **no** repo segment). That file is **auto-generated** on **`prebuild`** via **`abey-sync-spa-paths`** (**`@abeyjs/view`**) and **`docs/web/abey-spa-paths.config.json`** — AST scans of route sources (**no** OM **`.html`** in **`vite.config`**, which would break **esbuild** without the Abey plugin).

**When you add or remove a user-facing docs URL**, change those TS route files / **`files`** in the JSON as needed; **`npm run docs:spa-paths:sync -w abeyjs-docs-web`** (**`npm run build`** runs it automatically) refreshes **`vite-doc-spa-paths.ts`**. Omit **`/`** (already **`dist/index.html`**) and **`*`** (in-app fallback only). Unknown URLs continue to rely on **`404.html`** plus the shell’s internal not-found page.
...
| **`vite-doc-spa-paths.ts`** | **Generated** SPA path list for **`docs-spa-html-fallback-dirs`** (**`abey-sync-spa-paths`**, **`abey-spa-paths.config.json`**, **`prebuild`**). Do not edit by hand. |
